...
def main():
    ...
    gpu_analyzer = GPUUtilizationAnalyzer(base_path)
    ...
    # Generate GPU utilization analysis
    print("\nLoading GPU utilization data...")
    gpu_data = gpu_analyzer.load_all_gpu_data()
    
    if not gpu_data.empty:
        print("Calculating GPU utilization metrics...")
        utilization_df = gpu_analyzer.calculate_gpu_utilization(gpu_data)
        summary_df = gpu_analyzer.calculate_time_weighted_utilization(utilization_df)
        
        print("Generating GPU utilization graphs...")
        fig6 = visualizer.plot_gpu_utilization_over_time(utilization_df)
        fig6.savefig(os.path.join(output_dir, "gpu_utilization_over_time.png"), dpi=300, bbox_inches='tight')
        plt.close(fig6)
        
        fig7 = visualizer.plot_model_gpu_utilization(utilization_df)
        fig7.savefig(os.path.join(output_dir, "model_gpu_utilization.png"), dpi=300, bbox_inches='tight')
        plt.close(fig7)
        
        fig8 = visualizer.plot_gpu_utilization_summary(summary_df)
        fig8.savefig(os.path.join(output_dir, "gpu_utilization_summary.png"), dpi=300, bbox_inches='tight')
        plt.close(fig8)
    ...
    for filename in ["total_violations_line.png", "total_violations_bar.png", 
                     ...
                     "gpu_utilization_over_time.png", "model_gpu_utilization.png", "gpu_utilization_summary.png"]:
        print(f"  - {filename}")
